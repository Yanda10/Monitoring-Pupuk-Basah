...
import { Flame, Beaker, Database, Download, Trash2 } from "lucide-react";
...
export default function Home() {
  ...
  const handleClearAll = async () => {
    const confirmDelete = confirm("Apakah kamu yakin ingin menghapus SELURUH riwayat log?");
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white text-slate-900 p-6 rounded-2xl shadow-lg border border-slate-100 flex flex-col justify-between">
            ...
            <div className="text-5xl font-black mt-4">{Number(latest.ph || 0).toFixed(2)}</div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-rose-50 hover:bg-rose-100 text-rose-600 disabled:opacity-50 text-sm font-medium rounded-xl transition"
              >
              ...
              <button
                onClick={exportToCSV}
                disabled={data.length === 0}
                className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 disabled:opacity-50 text-sm font-medium rounded-xl transition"
              >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-400 text-xs font-semibold uppercase tracking-wider">
                  ...
                  <th className="p-4">Konsentrasi (PPM)</th>
